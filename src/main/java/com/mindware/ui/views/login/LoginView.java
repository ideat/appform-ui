...
//        userRestTemplate = new UserRestTemplate();
        ...
        component.addLoginListener(e ->{
            if(authenticationType.equals("jwt")) {
                loginJwt(component, e);
            }else{
                loginLdap(component,e);
            }
    ...
    private void loginJwt(LoginForm component, AbstractLogin.LoginEvent e) {
...
//                VaadinSession.getCurrent().getSession().setMaxInactiveInterval(60000);
    ...
    private void loginLdap(LoginForm component, AbstractLogin.LoginEvent e) {
        JwtRequest jwtRequest = new JwtRequest();
        jwtRequest.setUsername(e.getUsername());
        jwtRequest.setPassword(e.getPassword());
        try {

            Token token = restTemplate.getTokenLdap(jwtRequest);
            String loginUser = e.getUsername();
//                VaadinSession.getCurrent().getSession().setMaxInactiveInterval(60000);
            VaadinSession.getCurrent().setAttribute("jwt", token.getToken());

            Users users = userRestTemplate.findByAdUser(loginUser);
            AdusrOfi adusrOfi = adusrOfiRestTemplate.findByLogin(users.getLogin());
            Gbpmt gbpmt = gbpmtRestTemplate.findAll();
            VaadinSession.getCurrent().setAttribute("jwt", null);
